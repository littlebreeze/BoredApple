...
export default function TimerWrapper({ roomId }: { roomId: string }) {
  // 효과음
  const clockES = effectSound(clockSound, 0);
  const timerES = effectSound(timerSound, 0);
  const hintES = effectSound(hintSound, 0);
  const correctES = effectSound(correctSound, 0);
  const timeOutES = effectSound(timeOut, 0);
