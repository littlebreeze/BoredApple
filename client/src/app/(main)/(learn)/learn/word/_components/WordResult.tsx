...
import { IWordProblem, WordProblemResponse } from '@/types/Problem';

export default function WordResult() {
  const router = useRouter();
  const [wordProblems, setWordProblems] = useState<WordProblemResponse>([]);
                ...
                <div className='w-96 bg-white rounded-xl p-4'>
                  {[1, 2, 3].map((option) => (
                    ...
                    >
                      ...
                      <span>{currWordProblem[`option${option}` as keyof IWordProblem]}</span>
                    </div>
                  ))}
                </div>
              </div>
